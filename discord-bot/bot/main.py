...
from .validation import ValidationError, validate_discord_attachment
...
def build_bot(config: Config):
    ...
    @tree.command(name="animegen_i2v", description="選圖後開啟 AnimeGen I2V 互動設定", guild=guild)
    @app_commands.describe(image="起始圖片；其餘選項會在下一步彈窗輸入")
    async def animegen_i2v(
        ...
        await interaction.response.send_modal(
            FixedVideoModal(api, mode="animegen", image=image)
        )
