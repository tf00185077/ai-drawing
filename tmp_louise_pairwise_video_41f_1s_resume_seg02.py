...
SEG01_SRC = Path('/Users/tf00185088/comfyui/output/video/louise_pairwise_wand_motion_seg01_q4ks_512x704_41f41fps_00001_.mp4')
SEG01_DST = FINAL_DIR / 'louise_pairwise_wand_motion_seg01_q4ks_512x704_41f41fps.mp4'
...
def output_from_history(pid):
    ...
    for node_id,node_out in item.get('outputs',{}).items():
        for key in ('videos','gifs','animated','files','images'):
...
def main():
    ...
    rels=stage_inputs()
    if not SEG01_SRC.exists() and not SEG01_DST.exists():
        raise FileNotFoundError(f'missing completed seg01: {SEG01_SRC}')
    FINAL_DIR.mkdir(parents=True, exist_ok=True)
    if not SEG01_DST.exists():
        shutil.copy2(SEG01_SRC, SEG01_DST)
        log(f'seg01 copied {SEG01_DST}')
    else:
        log(f'seg01 already copied {SEG01_DST}')

    prefix='louise_pairwise_wand_motion_seg02_q4ks_512x704_41f41fps'
    wf=configure(rels[1], rels[2], prefix, 482910042)
    (FINAL_DIR/f'{prefix}_submitted_workflow.json').write_text(json.dumps(wf,indent=2),encoding='utf-8')
    out,pid=submit_and_wait(wf, 'seg02')
    seg02_dst=FINAL_DIR/f'{prefix}{out.suffix}'
    shutil.copy2(out,seg02_dst)
    log(f'seg02 copied {seg02_dst}')
    concat([SEG01_DST, seg02_dst])
