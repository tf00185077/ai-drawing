...
TASK='louise_white_orb_fullscreen_growth_14b_20260701'
...
END=G/'louise_white_orb_fullscreen_end_keyframe.png'
SEG_ORB=G/'louise_14b_tiny_white_dot_expand_to_fullscreen_white_orb_512x704_33f16fps.mp4'
FINAL=G/'louise_14b_seg01_v1_extend_white_orb_fullscreen_concat.mp4'
CONTACT=G/'louise_14b_white_orb_fullscreen_growth_contact.jpg'
PROMPT=(
 ...
 'a tiny white point of light just beyond the wand tip expands outward into a large pure white glowing magic orb, the orb rapidly grows larger and larger until it covers the entire screen, '
 'full screen white magical flash at the end, expanding circular white light, radial glow, soft bloom, no hard cut, continuous outward growth, '
 'preserve character, wand, and castle position at the beginning, then the expanding white orb naturally obscures the character and background, no camera move, no zoom'
)
NEG=(
 'low quality, blurry, jitter, heavy flicker, motion blur, sudden cut, black explosion, orange fire, red fireball, smoke cloud, debris, dark blast, huge black smoke, '
 'morphing face before covered, distorted hands before covered, changed outfit before covered, duplicate character, camera shake, camera move, zoom, pan, text, watermark, logo, signature'
)
...
def configure(start_rel,end_rel):
    ...
    for nid in ('86','85'):
        ...
        w[nid]['inputs']['noise_seed']=915501
    w['86']['inputs']['start_at_step']=0
    w['86']['inputs']['end_at_step']=3
    w['85']['inputs']['start_at_step']=3
    w['85']['inputs']['end_at_step']=6
    w['95']['inputs']['model_name']='Wan2.2-I2V-A14B-HighNoise-Q4_K_S.gguf'
    w['96']['inputs']['model_name']='Wan2.2-I2V-A14B-LowNoise-Q4_K_S.gguf'
    w['108']['inputs']['filename_prefix']='video/louise_14b_tiny_white_dot_expand_to_fullscreen_white_orb_512x704_33f16fps'
    return w
...
def submit_wait(w):
    ...
    log(f'white orb transition submitted prompt_id={pid}')
    start=time.time(); last=-1
    while True:
        out,status=output_from_history(pid)
        if out:
            log(f'white orb transition completed output={out} status={status}')
            return out
...
def concat():
    listfile=G/'louise_14b_seg01_v1_extend_white_orb_fullscreen_concat_list.txt'
...
def contact():
    tmp=Path('/tmp/louise_white_orb_growth_contact'); shutil.rmtree(tmp,ignore_errors=True); tmp.mkdir(parents=True)
...
def main():
    ...
    end_rel=stage(END,'fullscreen_white_orb_end.png')
