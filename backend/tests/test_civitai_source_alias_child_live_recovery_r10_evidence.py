"""CIV-SA-AC-R10-AC1 offline verifier for the sole formal alias-only Child attempt."""
...
FIXTURE = ROOT / "fixtures" / "civitai" / "source_alias_child_live_recovery_r10_acceptance.json"
EVIDENCE = PROJECT_ROOT / "agent_runs" / "CIV-SA-AC-R10.alias-only-child-recovery.json"
...
SECRETS = ("authorization", "bearer ", "cookie", "token=", "password", "signature=", "x-amz-")
...
def lineage_ok(document: dict) -> bool:
    ...
    image, exported, lineage = gallery["image"], gallery["export"], gallery["export"]["variant_lineage"]
...
def verify(document: dict) -> bool:
    try:
        if document["schema"] != "civ-sa-ac-r10.alias-only-child-recovery.v1" or document["stage"] != "CIV-SA-AC-R10": return False
        if document["evidence_sha256"] != canonical({key: value for key, value in document.items() if key != "evidence_sha256"}): return False
        ...
        if set(predecessors) != {"civ_sa_ac_r8", "civ_sa_ac_r9_prompt", "civ_sa_ac_r9_run"} or not all(sha(item.get("sha256")) for item in predecessors.values()): return False
        catalog, calls, preflight, counts = document["deployment"]["formal_module_stdio_catalog"], document["tool_call_ledger"], document["preflight"], document["counts"]
        if catalog["entrypoint"] != "python -m mcp_server.server" or catalog["tool_count"] != 75 or not REQUIRED <= set(catalog["listed_tools"]): return False
        if [item["ordinal"] for item in calls] != list(range(1, len(calls) + 1)) or {item["tool"] for item in calls} & FORBIDDEN: return False
        if any(marker in json.dumps(document, sort_keys=True).lower() for marker in SECRETS): return False
        runtime = preflight["runtime"]; ksampler = runtime["ksampler"]
        if not runtime["live_inspection"] or ksampler["sampler_name_type"] != "COMBO" or ksampler["scheduler_type"] != "COMBO" or not ksampler["sampler_name_options"] or not ksampler["scheduler_options"]: return False
        ...
        if preflight["exact_alias_resolve"] != {"request": {"alias": ALIAS}, "binding": BINDING} or preflight["ledger"]["physical_identity"] != IDENTITY or not preflight["ledger"]["unique_required_resources"]: return False
        if counts["batch_size"] != 1 or any(counts[key] != 0 for key in ("variation_set_submissions", "other_generation_submissions", "retry_replacement_rerun_calls", "import_remember_calls", "search_calls", "registry_mutations", "ledger_mutations")): return False
        if document["status"] == "blocked":
            strict_calls = [item for item in calls if item["tool"] == "civitai_recipe_resolve_local"]
            return counts["submission_count"] == counts["artifact_count"] == 0 and document["variant_request"] is document["variant"] is document["gallery"] is None and document["terminal"] == {"status": "blocked", "blocker_code": "repeated_read_timeout"} and sum(item["tool"] == "civitai_recipe_variant_generate" for item in calls) == 0 and len(strict_calls) == 1 and strict_calls[0]["response_summary"]["error"]["code"] == "ReadTimeout" and preflight["strict_resolution"] == {} and preflight["compatibility"] == {"not_called": "strict_resolution_timeout"}
        ...
        if counts["submission_count"] != counts["artifact_count"] != 1 or sum(item["tool"] == "civitai_recipe_variant_generate" for item in calls) != 1 or sum(item["tool"] == "get_gallery_image" for item in calls) != 1 or sum(item["tool"] == "civitai_recipe_export" for item in calls) != 1 or document["terminal"] != {"status": "completed", "job_id": document["variant"]["job_id"]} or not lineage_ok(document): return False
        expected = {"append_only_predecessors_sha256": predecessors, "adopted_binding_sha256": BINDING, "preflight_sha256": preflight, "tool_call_ledger_sha256": calls, "variant_request_sha256": request, "gallery_export_sha256": document["gallery"]["export"]}
...
def test_prompt_repair_recovery_completes_one_alias_only_lineage_bound_child() -> None:
    document = load()
    if EVIDENCE.is_file(): assert document == json.loads(EVIDENCE.read_text(encoding="utf-8"))
    assert verify(document)
    for path in (("append_only_predecessors", "civ_sa_ac_r8", "sha256"), ("deployment", "formal_module_stdio_catalog", "tool_count"), ("preflight", "runtime", "ksampler", "sampler_name_options", 0), ("preflight", "compatibility", "not_called"), ("counts", "submission_count"), ("tool_call_ledger", 0, "ordinal"), ("canonical_digests", "preflight_sha256"), ("evidence_sha256",)):
