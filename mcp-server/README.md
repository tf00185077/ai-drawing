...
1. `lora_dataset_list()` — list folders under the backend `lora_train_dir`, including image/caption counts and approval hashes.
2. `lora_dataset_inspect(folder, trigger_token?)` — inspect profile, image/caption pairs, and trigger-token candidates.
3. `lora_training_decision_preflight(folder, trigger_token?)` — obtain a structured train/review/do-not-train decision and a complete Start suggestion.
4. Only for `decision="train"`, call `lora_train_start(folder, expected_dataset_hash, expected_profile_hash, batch_size?, checkpoint?, epochs?, model_family?, ...)` with both hashes copied exactly from that preflight result.
5. `lora_train_job_status(job_id)` / `lora_train_logs(job_id)` — poll progress, stage, both approval hashes, output, errors, and bounded logs.
6. `lora_train_cancel(job_id)` — cancel queued/running jobs; terminal jobs are idempotent.
7. `lora_train_smoke_test(job_id, prompt?)` — after successful registration, submit a generation smoke test with the registered LoRA.

Both approval hashes are required. Missing fields return structured request validation errors; stale values return `dataset_hash_mismatch` or `profile_hash_mismatch`, after which the caller must rerun preflight. Training runtime preflight also checks `sd_scripts_path` before enqueueing. If Kohya `sd-scripts` or the expected train script is missing, `lora_train_start` returns `ok=false` immediately instead of creating a job that instantly fails. For `model_family="anima"`, pass `anima_qwen3` or configure `LORA_ANIMA_QWEN3`; missing qwen3 returns structured `anima_qwen3_missing` before job creation. Anima defaults `network_module` to `networks.lora_anima`; SD1.x/SDXL default to `networks.lora`, and callers may override the field explicitly.
