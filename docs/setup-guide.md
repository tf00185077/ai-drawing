...
設定寫入採 validation-before-replace；任一替換失敗會復原整組舊檔。Compose 或 readiness 失敗時，launcher 會停止本次新建且 ownership 可證明的資源並回復原設定。安裝 staging 使用每次唯一且具 ownership identity 的 sibling 暫存目錄，不會清除既有的 `.staging` 或名稱相似目錄。

`update-comfyui` 必須在 managed ComfyUI 停止後執行。更新器先把 launcher 管理的舊 `.venv` 原子移到每次唯一的 backup，再 checkout 固定版 source，於另一個唯一目錄建立並 smoke-test 全新 `.venv`，最後才啟用。任一步驟失敗時，會依序恢復舊 commit、原 `.venv` 並 smoke-test 舊 runtime；三者全部成功才回報 `COMFYUI_UPDATE_FAILED_RESTORED`。如果 rollback 不完整，回報 `COMFYUI_UPDATE_ROLLBACK_FAILED` 並保留 recovery backup，不會宣稱已恢復。它不會刪除非空使用者目錄，也不會只憑 PID 終止程序。
...
若回報 `COMFYUI_UPDATE_ROLLBACK_FAILED`，不要啟動 ComfyUI，也不要刪除訊息指出的 recovery backup；先檢查 `data/logs/bootstrap.log` 與 managed ComfyUI 目錄。
