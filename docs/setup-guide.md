...
沒有指定指令時，第一次等同 `setup`；已有設定時等同 `start`。wrapper 會在使用者 cache 內取得固定版 uv `0.11.29` 與 Python `3.12`，然後執行共用 Python launcher，不污染全域 Python/Node 環境。macOS/Linux 的 `UV_UNMANAGED_INSTALL` 是版本目錄本身，installer 與 wrapper 都使用其直接子檔 `uv`（不是 `bin/uv`）；Windows 同樣使用版本目錄直接子檔 `uv.exe`。
...
11. 顯示實際網址與 ComfyUI 模式；失敗時回復舊設定，並清理本次新啟動且 ownership 可驗證的程序。若 spawn 後初始 identity 尚不可得，只透過 exact `Popen` handle 做 bounded terminate/wait/kill 並驗證退出，絕不對未驗證 PID 發 signal。
...
| `./setup.sh update-comfyui` | 只更新 launcher 安裝且 provenance 完整、目前已停止的 ComfyUI；使用者自有目錄不會被修改。 |
...
CLI 依主機 probe 與可確認的模型數輸出 `not_configured`、`unreachable`、`no_models` 或 `connected`，另外呈現 ownership、模型數與 relay。CLI 不輸出 `degraded`；`degraded` 是 Backend API / Dashboard 對目錄警告的彙總狀態。Backend 在 filesystem inventory 為零或不可用時，會以 2 秒 timeout、2 MiB 上限查詢 ComfyUI `/object_info`，只解析 `CheckpointLoaderSimple.ckpt_name` 與 `UNETLoader.unet_name` 並去重；external instance 的 live inventory 有模型即為 `connected`，HTTP／JSON／schema failure 則維持既有保守狀態且 status endpoint 繼續回應。
...
通過前置檢查後，更新器先把 launcher 管理的舊 `.venv` 移到每次唯一的 backup，再 checkout 固定版 source，於另一個唯一目錄建立並 smoke-test 全新 `.venv`，最後才啟用。lifecycle lock 排除其他 launcher 進入這段 mutation。若新 `.venv` 已啟用後才發生錯誤，rollback 不會再做「先檢查 pathname、再搬移 pathname」的非原子操作；它會保留目前 `.venv` 與舊 backup，並誠實回報 `COMFYUI_UPDATE_ROLLBACK_FAILED`，交由人工檢查。

任一步驟失敗時，只有舊 commit、原 `.venv` 與舊 runtime smoke 全部可證明恢復後，才會回報 restored outcome；任何無法安全證明的 rename/rollback 都是 `COMFYUI_UPDATE_ROLLBACK_FAILED`。因為 temp 永不自動刪除，核心恢復成功通常會搭配 `COMFYUI_UPDATE_FAILED_RESTORED_CLEANUP_PENDING` 與精確保留路徑。更新成功時，會先保存新的 `installed_commit` 與 stopped provenance，再顯示 `COMFYUI_UPDATE_CLEANUP_PENDING` warning；若 filesystem 更新已成功但 state 保存失敗，則改以 `COMFYUI_UPDATE_SUCCEEDED_STATE_SAVE_FAILED` 明確回報新版本與所有 pending backup/new-env path，不會包成 generic `OSError`。

所有平台都會保留仍存在且唯一命名的 `.ComfyUI.staging-*`、`.ComfyUI.venv-backup-*` 或 `.ComfyUI.venv.update-new-*`。確認沒有 install/update/rollback 正在執行後，才可依 warning 的精確路徑人工檢查與移除。launcher 不會自動刪除這些目錄，即使它們看起來是空的。
...
更新不能覆寫執行中或 ownership 無法確認的 `.venv`。請先讓 `stop` 以完整 PID identity 安全停止或清除 stale ownership，再更新並重新啟動：
...
若回報 `COMFYUI_UPDATE_ROLLBACK_FAILED`，不要啟動 ComfyUI，也不要刪除訊息指出的 recovery backup；先檢查 `data/logs/bootstrap.log` 與 managed ComfyUI 目錄。若只回報 `COMFYUI_UPDATE_CLEANUP_PENDING` 或 `COMFYUI_UPDATE_FAILED_RESTORED_CLEANUP_PENDING`，核心更新或還原已完成，但保留路徑仍需在確認沒有操作進行後人工處理。

若回報 `COMFYUI_START_ORPHAN_CLEANUP_FAILED` 或 `RELAY_START_ORPHAN_CLEANUP_FAILED`，launcher 已用 exact child handle 嘗試 terminate/kill，但仍無法確認退出。不要依名稱猜測或終止其他 PID；記下 hint 內的 spawned PID，先執行 `status`，檢查對應 `comfyui.log`／`comfyui-relay.log` 與系統程序資訊後再人工處理。
