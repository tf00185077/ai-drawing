...
設定寫入採 validation-before-replace；任一替換失敗會復原整組舊檔。Compose 或 readiness 失敗時，launcher 會停止本次新建且 ownership 可證明的資源並回復原設定。安裝 staging 使用每次唯一且具 ownership identity 的 sibling 暫存目錄，不會清除既有的 `.staging` 或名稱相似目錄。cleanup 採 fail-closed：只有身分仍一致的空目錄可用 `rmdir` 移除，非空目錄絕不自動遞迴刪除；安裝失敗時會以 `COMFYUI_INSTALL_CLEANUP_PENDING` 指出保留路徑。
...
通過前置檢查後，更新器先把 launcher 管理的舊 `.venv` 原子移到每次唯一的 backup，再 checkout 固定版 source，於另一個唯一目錄建立並 smoke-test 全新 `.venv`，最後才啟用。啟用後的 `.venv` 會記錄 device/inode；rollback 前若身分已被替換，絕不移動或刪除未知目錄，而是保留舊 backup 並回報失敗。

任一步驟失敗時，會依序恢復舊 commit、原 `.venv` 並 smoke-test 舊 runtime；三者全部成功且暫存清理也成功才回報 `COMFYUI_UPDATE_FAILED_RESTORED`。核心恢復成功但非空暫存依 fail-closed 規則保留時，回報 `COMFYUI_UPDATE_FAILED_RESTORED_CLEANUP_PENDING`，舊 runtime 仍可使用。rollback 不完整則回報 `COMFYUI_UPDATE_ROLLBACK_FAILED`。更新成功時，即使舊 backup cleanup pending，仍會先保存新的 `installed_commit` 與 stopped provenance，再顯示 `COMFYUI_UPDATE_CLEANUP_PENDING` warning。

Windows 與任何無法提供安全遞迴刪除保證的平台，都可能保留唯一命名的 `.ComfyUI.staging-*`、`.ComfyUI.venv-backup-*` 或 `.ComfyUI.venv.update-new-*`。確認沒有 install/update/rollback 正在執行後，才可依 warning 的精確路徑人工檢查與移除。launcher 不會自動遞迴刪除這些非空目錄。
