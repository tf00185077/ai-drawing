...
新需求下來時，按照以下四層拆解，由上往下展開：

```
GOAL  → 這個需求讓 agent 能多做到什麼？（能力描述）
  ↓
TASK  → 一個 task = 一個完整的 agent 能力（端到端可測）
  ↓
STEP  → 有序的實作步驟，每步各自可驗證
  ↓
TEST  → 端到端驗收：agent 呼叫後結果正確
```

### 每一層的判斷標準

**GOAL**：用「agent 可以 ___」的句型描述。寫不出這個句型，代表需求還不夠清楚。

**TASK**：用「完成後 agent 多了一個操作」判斷邊界。
- ✅ 一個 task = 一個 agent 操作（如「查詢 job 狀態」）
- ❌ 不以技術層（schema / API / MCP）切割 task

**STEP**：按實作相依順序排列，通常是：DB → Core → API → MCP。每個 step 有獨立的 verify 指令，step 之間有方向性（後面的 step 依賴前面的 step）。

**TEST**：用 agent 的視角寫：「呼叫 MCP tool X，預期結果是 Y」。不是「跑 pytest 通過」，而是「功能實際可用」。
...
```
Task A: 加 schema 欄位     ← 做完沒有任何功能可用
Task B: 加 API endpoint    ← 做完有 API 但 agent 還不能用
Task C: 加 MCP tool        ← 做完才有功能，但已拆成三個 task
```

Task A、B 單獨完成不代表任何功能到位，無法獨立驗收。三個 task 之間有嚴格順序依賴，必須串行。
...
Task: 查詢 job 狀態
  ...
  Step 2: recording 寫入 job_id
  Step 3: API GET /job/{id}
  Step 4: MCP get_job_status tool
  TEST: 呼叫 get_job_status → 回傳狀態正確
```

一個 task 包含所有層，做完就能測，不需要等其他 task。

---

### 從需求到 task 的拆解步驟

1. **讀 GOAL.md**，確認這個需求是否在現有目標範圍內。若是新能力，先更新 GOAL.md。
2. **列出 agent 操作清單**：這個需求讓 agent 能多做哪些事？每件事就是一個 task 候選。
3. **確認現況**：每個 task 候選，逐層（DB → Core → API → MCP）確認哪些已存在、哪些缺少。
4. **寫 task spec**：只寫缺少的部分，依相依順序排 step，每步有 verify。
5. **更新 PROGRESS.md**：把新 task 加進待做清單。
...
### Step 1：列出 agent 操作清單，以能力切 task

依照「需求拆解流程」章節，先問：**這個需求讓 agent 能多做哪些事？**  
每一件事是一個 task，task 以能力為邊界，不以技術層為邊界。

確認每個 task 候選的 step 組成，通常為 DB → Core → API → MCP。  
依照各 step 的性質選擇 verify 方法：

| Step 性質 | 天然的 Verify 方法 |
|------|-------------------|
...
| 新增 MCP tool | Unit test（mock HTTP client） |
