...
> 實作說明：所有 HTTP >= 400（含 5xx / 503 / 404 / 422）由 `ApiClient` 統一拋 `BackendError`，
> 指令層預設「轉傳 backend 的 message」（比固定字串更貼近實際原因）；只有真正的連線失敗
> （httpx `ConnectError`/`TimeoutException`，非 `BackendError`）才顯示「後端連不上」。
> 上表的 5xx→「後端連不上」與 503→「佇列滿了」為原始意圖；實作選擇轉傳 message，兩者皆可接受。
