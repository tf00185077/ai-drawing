...
- [ ] `add-video-generation-mcp`（openspec change，2026-06-22）：backend / MCP MVP 已實作，剩 known-good 本機 ComfyUI video workflow 端到端驗證與 verified video template manifest 未完成。已具備 generic `GeneratedArtifact` lifecycle、artifact output detection（png/jpg/jpeg/webp/gif/mp4/webm）、queue 完成後複製 artifact 至 gallery、成功無可記錄 artifact 時 `recording_error` failed、`GET /api/gallery/artifacts/{id}`、generation status `artifacts[]`（保留 legacy `image_id` / `image_path`）、MCP `get_gallery_artifact`、MCP/API `generate_video_custom_workflow`、video input refs（`image` / `first_frame` / `last_frame` / `video_ref`）、video template tags（`txt2video` / `img2video` / `first_frame` / `last_frame` / `video_ref` / `audio_ref`）、video job artifact gate for `save_workflow_template`，以及 MCP docs 的 derivation loop / MVP out-of-scope guardrails。驗證：targeted backend `117 passed`、mcp-server full `81 passed`、OpenSpec strict pass、OpenSpec all `8 passed, 0 failed`；backend full `260 passed, 3 failed`（既有/無關：config path expectation、兩個 LoRA trainer legacy tests）。live video run 等 CTY 提供 known-good workflow；本次搜尋 `backend/workflows` 與 repo 內 video workflow 檔，未找到可提交的 known-good video workflow。
