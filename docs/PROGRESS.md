...
- Final fix wave 將 Windows PowerShell 5.1 argv 改為固定 ASCII UTF-8 stdin bootstrap；呼叫者 script 仍逐字只經 stdin 傳入、不進 argv、不建立暫存 script。Python／PowerShell 的 stdin、stdout、stderr 均為 strict UTF-8，`errors="replace"` 已移除；實機在 host CP932 下逐字驗證繁中、日文、emoji 的 multiline stdin/stdout/stderr。此介面只承諾 Unicode text streams，不承諾 arbitrary binary streams（binary 必須自行以 Base64 等文字編碼）。
- `cancel()` 現在不論發生在 process registration 前或後，都會在 response 返回前原子發布單一 `cancelled` terminal transition 與固定 `finished_at`；後續 exit/output capture 只能補 `exit_code/stdout/stderr`，不可覆寫 state/timestamp。
- Mac Backend 以 URL-only client（保留 hostname/protocol 的 URL discovery 與 runtime URL cache，但不作 request authorization）代理三個對應 routes：`POST /api/workers/powershell/commands`、`GET /api/workers/powershell/commands/{command_id}`、`POST /api/workers/powershell/commands/{command_id}/cancel`。三個 HTTP 503 failure response 現分別以回歸測試鎖定 `worker_powershell_submit_failed`、`worker_powershell_status_failed`、`worker_powershell_cancel_failed`。
- 一般產圖直接送 native payload 到 ComfyUI `/prompt`；live Worker 的 managed update/restart、resource plan/content 與 workflow preflight HTTP routes 已移除並回 404。Backend lifespan 不再 import/start legacy coordinator，legacy auto-update setting／token／applying state 均不會阻擋 `/prompt`。保留的本機 `Restart-Worker.ps1` 也不再讀 token、送 Bearer header 或等待 ComfyUI/Worker health/preflight；update/repair/restart 由 Mac 直接提交 open PowerShell script。
- `.env.example`、Settings default、Worker `/v1/worker/status` 與 URL discovery test contract 已統一為 protocol 2，避免範例環境在 discovery 時拒絕新 Worker。
...
- Final fix wave non-live verification：focused Worker/control gate `162 passed`；完整 Windows entrypoint/distribution gate `128 passed, 1 skipped`（PowerShell 5.1 shipped-script parser 與 real Unicode transport test 均有執行）；完整 Backend＋MCP 為 `1965 passed, 6 skipped, 17 failed, 76 warnings`。17 筆與前輪完全相同且與 open-control 無關：Prompt Library/comma-atomic 15 筆（`test_comma_atomic_inventory.py::{test_reviewed_repository_inventory_is_exact,test_reviewed_combination_projection_is_exact,test_retained_entries_are_byte_equivalent_metadata_records,test_derived_entries_match_reviewed_curation_and_source_invariants,test_migrated_combinations_preserve_metadata_and_exact_rendered_text}`、`test_comma_atomic_migration.py::{test_dry_run_is_locked_complete_and_byte_stable,test_apply_keeps_marker_and_second_dry_run_is_zero_change,test_finalize_refuses_until_atomic_enforcement_is_active,test_activate_finalize_and_rollback_guards,test_immediate_rollback_restores_preimages_and_stays_closed,test_rollback_refuses_diverged_post_image,test_post_finalize_divergence_refuses_rollback_and_restores_ready_state,test_apply_refuses_reviewed_plan_after_source_hash_drift,test_interrupted_apply_resumes_same_locked_plan}`、`test_prompt_library_seed.py::test_seed_catalog_is_complete_bilingual_and_clean`），multi-LoRA 2 筆（`test_multi_lora.py::{test_loras_takes_precedence_over_single_lora,test_extra_loras_ignored_without_error}`）。未修改或排除這些 baseline tests。
- Live retired-gate 與 stale transport pattern scans 均為零匹配。Windows 套件重建後 source／dist directory／ZIP 36 檔逐位元一致，含 `worker.py`、`powershell_control.py`、`Enable-Open-PowerShell-Control.ps1`、兩份 README 與 token/health-free `Restart-Worker.ps1`；ZIP SHA-256 `89ad293bf4027fad22f8f374deac0a2be81f0a0e51d438ac245ba87ca72c87a0` 僅為發行 metadata，未成為 runtime、deployment 或 command gate。push、merge、live enablement、scheduled-task、ComfyUI 或 GPU 操作均未執行。
...
- `Deploy-UpdaterBootstrap.ps1` 以 secured ProgramData exclusive lock 串行 transaction，只停止並等待 `AI-Drawing Worker Updater` task。固定的 `updater-bootstrap-deployment-owned\transaction-journal.json` 僅保存 schema version、exact commit 與 closed stage，以同目錄 atomic replace、durable flush 與 child ACL 保護；第一個 destructive move 前先寫入，四個 backup／activation moves 後逐步推進。下次啟動在 lock 內、normal installed-updater preflight 前先讀 journal；pre-recovery 只驗證 root 與精確 recovery-control path，避免 transitional installed updater／ProgramData child 阻擋復原。
- Worker 與 ProgramData 在各自 volume 以 whole-path backup／staging／switch 完成啟用，再套用 ACL、執行 installed updater import/config/recovery smoke，並只驗證既有 scheduled-task action 而不啟動 task。rollback 會複製完整 updater backup tree 與 bootstrap backup，不做 content compare、也不消耗 backup；成功 rollback 或 failed restored smoke 都保留兩份 backup。無法安全證明復原時回 `UPDATER_BOOTSTRAP_RECOVERY_REQUIRED`，journal 與所有存在的 evidence paths 均保留。lock handle 一律 dispose；stale lock cleanup 失敗只回固定 `UPDATER_BOOTSTRAP_LOCK_CLEANUP_WARNING`，不覆蓋 ready／rolled-back 結果或洩漏 raw error。
- 信任邊界仍是完整 Git commit 與 tracked-file allowlist；部署流程明確沒有 `Get-FileHash`、digest manifest、per-file content hash／內容相等比較或 source/staging/installed content gate。ZIP SHA-256 `72e2b5d6bcb80c30ad6eb20bca406591287430f1c4485122d354afcbdde7d0a1` 僅是下載／發行 metadata，不是部署 acceptance gate；source、dist directory 與 ZIP 都是 33 files。
- Elevated entrypoint 嚴格要求 Windows PowerShell Desktop 5.1。process-death tests 會在四個 destructive move 各自完成後強制終止 PowerShell，下一次 invocation 均在 installed preflight 前復原。updater-bootstrap focused gate 為 `71 passed, 58 deselected`；non-live full focused gate 為 `402 passed, 1 skipped`（2 個既有 warnings）；distribution／archive／updater-bootstrap parity 與 no-hash contract 子集為 `74 passed, 55 deselected`，`git diff --check` 通過。dist directory 與 ZIP 均已包含更新後 script 與 runbook。
...
- 再次 Setup 的 ACL gate 抓出 login recovery import 在 privileged `updater` tree 生成 user-owned `__pycache__`。啟動器改以 Python `-B -m updater.recovery` 執行，杜絕低權限 bytecode 寫入；既有 cache 只需精確刪除一次，updater tree 的 SYSTEM ownership invariant 即可持續成立。
- Setup 首次完整成功後，實機健康檢查再發現正式登入啟動器把 `AI_DRAWING_WORKER_PARTIAL_ROOT` 設成 shared 實體目錄，但 Worker managed contract 要求 release 內受驗證、指向 shared cache 的 `cache\.partial` junction；排程因此以結果碼 1 結束，只留下 ComfyUI。`Start-Worker.ps1` 已與 updater staged validation 對齊使用 release junction，並新增 launcher 契約回歸測試。
- C 槽 Worker 與 CUDA health 通過後，D 槽 migration 實機發現 source inventory 仍假設 flat legacy layout，會拒絕 Setup 已建立的合法 managed `current` 與 release shared junction。Inventory 現在只略過經精確驗證的 `current -> releases/<commit>` 及五個固定 shared links；target 必須留在同一 Worker root 且完全匹配，未知／外部 reparse point 原測試仍 fail-closed。Windows focused gate `167 passed, 1 skipped`，distribution 已重建並逐位元 parity 通過；ZIP SHA-256 `35d6728af26d3c1a727e6894e55633c0d328add8791a09d0d18e932565f57552`。
- 後續實機 inventory 又證實 active release 的 `ComfyUI/user/comfyui.db` 會被運行中的 ComfyUI 鎖定；source `releases` 本來也不在 migration copy set（D 槽 release 由 flat runtime、shared 與 payload 重建），不應重複 hash。Managed source 現改為先驗證 `current` target 的 direct-child commit 路徑、`source-commit.txt` 與 `.managed-release.json`，再排除整個舊 `releases`；實際搬移的 runtime/shared/config 仍逐檔 hash，外部 reparse 測試維持拒絕。
- README 的 D 槽正式 migration 命令同步修正為 `C:\AI-Drawing-Worker-Source\worker\windows\Migrate-Worker.ps1`。Focused fake-uv／真實 Windows junction／CUDA wheel ordering 與 activation gate RED→GREEN；首輪實機另抓出 manifest minor selector `3.12` 與實體 patch 目錄 `3.12.13` 的差異，補為 minor selector 僅匹配唯一 `3.12.x`、完整 patch selector 仍精確匹配。Worker focused gate：`267 passed, 1 skipped`，Python compileall 與 `git diff --check` 通過。source/dist/ZIP 共 28 檔逐位元一致；bytecode-safe recovery 第六版修正 ZIP SHA-256：`ebaa91cfb94833fa95cb852721a31be9455f91907940c9333a17db6da8aa0944`。實機 Setup/migration 結果待本節後續補記。
