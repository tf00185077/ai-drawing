...
- [ ] `add-video-generation-mcp`（openspec change，2026-06-22）：完成 CTY 核准的非互動 brainstorming/design 更新，尚未實作 backend/MCP code。新增 `docs/superpowers/specs/2026-06-22-video-generation-mcp-mvp-design.md`，並重整 `openspec/changes/add-video-generation-mcp/` 的 proposal/design/specs/tasks：MVP 聚焦 `GeneratedArtifact` 或等效 artifact lifecycle、status `artifacts[]`、`get_gallery_artifact`、`generate_video_custom_workflow`、video template capability tags，以及以 known-good ComfyUI video workflow 做真實端到端驗證。已執行 `openspec validate add-video-generation-mcp --strict` 與 `openspec validate --all`，皆通過。
