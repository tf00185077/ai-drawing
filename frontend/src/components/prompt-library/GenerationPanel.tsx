...
export default function GenerationPanel({ forms, positivePrompt, negativePrompt, preflight }: { forms: GenerationForm[]; positivePrompt: string; negativePrompt: string; preflight: () => string | null }) {
  ...
  async function generate() {
    ...
    const response = await fetch("/api/generate/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ template: workflow, prompt: positivePrompt, negative_prompt: negativePrompt, use_workflow_defaults: true, seed_mode: seedMode, execution_target: executionTarget }) });
  ...
  return (
    <section className="rounded-xl border border-slate-700 bg-slate-900/70 p-5">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_180px_180px_auto]">
