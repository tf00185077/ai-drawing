...
export default function PromptEntryBrowser({ categories, activePolarity, onPolarityChange, selectedCategory, entries, onOpenCategory, onAddEntry, onAddLiteral }: Props) {
  ...
  return (
    <section className="h-fit rounded-xl border border-slate-700 bg-slate-900/70 p-5">
      ...
      <ul className="mt-4 space-y-2">{visibleEntries.map((entry) => <li key={entry.id} className="flex items-start justify-between gap-3 rounded-lg bg-slate-800 p-3"><div><p className="font-medium text-white">{entry.name_zh}</p><p className="mt-1 text-xs text-slate-400">{entry.prompt}</p></div><button type="button" aria-label={`加入 ${entry.name_zh}`} onClick={() => onAddEntry(entry)} className="shrink-0 rounded-md bg-emerald-600 px-3 py-1.5 text-sm text-white">加入</button></li>)}</ul>
