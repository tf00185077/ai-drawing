...
TASK = 'louise_pairwise_wand_motion_41f2s_20260630'
PREV_FINAL = FINAL_DIR / 'louise_pairwise_1to2_2to3_wan22_q4ks_512x704_41f41fps_concat.mp4'
...
PROMPT = (
    'smooth two-second transition between the provided start and end keyframes, natural slower in-between motion, '
...
def configure(start_rel, end_rel, prefix, seed):
    ...
    # 41 frames / 20.5 fps = 2.0 seconds
...
def wait_for_previous():
    log(f'waiting for current 1s concat: {PREV_FINAL}')
    start=time.time(); last=-1
    while not PREV_FINAL.exists():
        mins=int((time.time()-start)//60)
        if mins != last:
            q=get_json('/queue')
            log(f'waiting previous {mins}m running={len(q.get("queue_running",[]))} pending={len(q.get("queue_pending",[]))}')
            last=mins
        time.sleep(30)
    log(f'previous 1s concat detected: {PREV_FINAL}')

def concat(paths):
    FINAL_DIR.mkdir(parents=True, exist_ok=True)
    final=FINAL_DIR/'louise_pairwise_1to2_2to3_wan22_q4ks_512x704_41f20p5fps_2s_concat.mp4'
    listfile=FINAL_DIR/'louise_pairwise_41f20p5fps_2s_concat_list.txt'
...
def main():
    ...
    rels=stage_inputs()
    segs=[]
    for idx,(a,b,seed) in enumerate([(rels[0],rels[1],582910041),(rels[1],rels[2],582910042)], 1):
        prefix=f'louise_pairwise_wand_motion_2s_seg{idx:02d}_q4ks_512x704_41f20p5fps'
        wf=configure(a,b,prefix,seed)
        (FINAL_DIR/f'{prefix}_submitted_workflow.json').write_text(json.dumps(wf,indent=2),encoding='utf-8')
        out,pid=submit_and_wait(wf, f'2s_seg{idx:02d}')
        dst=FINAL_DIR/f'{prefix}{out.suffix}'
        shutil.copy2(out,dst)
        log(f'2s_seg{idx:02d} copied {dst}')
