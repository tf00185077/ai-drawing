...
## Purpose
Define the agent-facing MCP tool surface for LoRA training: the structured-JSON result contract and the
decision-and-train loop (caption, dataset list/inspect, decision preflight, start, status, logs, cancel,
smoke test). Dataset preparation, validation, caption assessment, metadata profiles, and curation are
intentionally kept on the backend HTTP API rather than the MCP surface.
## Requirements
