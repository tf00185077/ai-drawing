...
- Define one weighted segment model: `snapshotRaw` is the exact unweighted Backend snapshot, `weight` is structured metadata, `renderedRaw=render(snapshotRaw, weight)`, and final text is `renderedAtoms.join(",")`. Frontend and Backend use the same at-most-three-decimal canonical weight form before sending or comparing. Textarea reconciliation compares exact `renderedRaw`; editing a rendered atom demotes it to a weight-1 literal whose `snapshotRaw` is the exact edited segment.
...
- Deploy a migration-compatible Backend gate before touching data. It recognizes the actual predecessor schema-v1 manifest even though that manifest has no comma-atomic flags, creates the marker, reports readiness false, and blocks all ordinary Prompt Library API/MCP reads and writes, including catalog/list/search/category/entry reads, combination load, and compose. Only privileged migration audit/dry-run/apply/resume/rollback operations may access staged state under the migration lock.
...
- Repair old saved combinations and one-to-many source references without silently losing provenance. An unresolved legacy ref is a blocking document diagnostic tied to its polarity and fallback positions: Update and Save As remain rejected until reviewed mapping, diagnostic-specific replacement-source selection that actually replaces those fallback atoms, or an explicit Backend-issued acknowledge-convert-to-literals token resolves it. Ordinary edit/save or an unrelated existing ref is not acknowledgment.
...
6. Activate and verify atomic Backend enforcement, including comma-free entry writes and canonical weighted compose/repair. Only then may one guarded finalize transition remove the marker and set readiness true; the run-specific rollback artifact remains available during the post-finalize rollout window and still requires exact post-image hashes.
...
- A post-finalize rollback could overwrite a later valid edit. The privileged rollback path temporarily fails closed, verifies every recorded post-image hash before replacing anything, restores ready state unchanged on divergence, and enters `rolled_back_required` only after an exact restore.
