...
W,H=832,1216
POSE='2026-06-30/louise_pose_skeleton_frame01_down_smaller_lowerleft.png'
STYLE_PROMPT=(
...
"academy uniform, white blouse, black capelet, pleated skirt, matching knee-high socks, shoes, tiny full body, complete head-to-toe visible, "
"three-quarter side view, very small character in the lower left corner, character occupies only the lower-left area and no more than two cells of a 3x3 grid, large empty scene around her, wide establishing shot, zoomed far out, "
"both arms lowered naturally, right hand holds a short straight wood-colored wand, plain wooden wand with no branches, no decoration, left hand empty, "
"large cartoon style European castle background dominating the whole image, huge complete European castle visible across the full frame, castle much larger than character, pointed spire towers, stone walls, rooftops, arched windows, full castle silhouette from left to right, full scene composition"
)
STYLE_SUFFIX=(
"soft backlight, pastel cyan pink blue yellow palette, watercolor wash background, paint splatter particles, floating petals, luminous airy negative space, flower-field color accents, silhouette-friendly lighting"
)
...
"photorealistic, oil painting, thick impasto, dark gothic palette, hard cel shading, close-up, upper body, cropped body, cropped feet, character centered, character too large, big character, medium shot, portrait, "
"small castle, cropped castle, incomplete castle, castle only on one side, empty background, modern building, front view, back view, raised arm, spell casting, explosion, magic effect, night sky, mismatched socks, bare legs, ornate wand, curved wand, thick wand, branch wand, forked wand, gem wand, glowing wand, staff, sword, second wand, extra arms, extra hands, bad hands, duplicate character, multiple people"
)
...
def workflow(prefix,prompt,negative,seed):
    return {
      ...
      "15":{"class_type":"AnimaLLLiteApply","inputs":{"model":["10",0],"lllite_name":"anima-lllite-pose-1.safetensors","image":["14",0],"strength":1.85,"start_percent":0.0,"end_percent":1.0,"preserve_wrapper":True}},
      "7":{"class_type":"KSampler","inputs":{"seed":seed,"steps":30,"cfg":5.4,"sampler_name":"er_sde","scheduler":"simple","denoise":1.0,"model":["15",0],"positive":["5",0],"negative":["6",0],"latent_image":["4",0]}},
...
jobs=[]
for i,seed in enumerate([27063401,27063402,27063403],1):
    prefix=f'louise_frame01_smaller_bigger_castle_cand{i:02d}'
    payload={'workflow':workflow(prefix,POS,NEG,seed),'prompt':POS,'negative_prompt':NEG,'seed':seed,'steps':30,'cfg':5.4,'width':W,'height':H,'sampler_name':'er_sde','scheduler':'simple','lora':'posing-dynamics-anima.safetensors','lora_strength':0.65,'diffusion_model':'anima_preview3Base.safetensors','text_encoder':'qwen_3_06b_base.safetensors','vae':'qwen_image_vae.safetensors','image_pose':POSE}
