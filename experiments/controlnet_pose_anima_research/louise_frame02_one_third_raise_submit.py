...
POSE='2026-06-30/louise_lowerleft_frame02_right_wand_hand_one_third_raise_pose_ref.png'
STYLE_PROMPT=(
...
"frame 2 of a five-frame wand raising sequence, her right hand is the wand hand and remains the only hand holding the wand, the same right hand is raised clearly upward from the lowered position to about one-third of the final casting angle, "
...
"left hand holding wand, wand switched hands, wrong hand holding wand, both hands holding wand, hand still fully lowered, raised arm too high overhead, spell casting, explosion, magic effect, ornate wand, curved wand, thick wand, branch wand, forked wand, gem wand, glowing wand, staff, sword, second wand, extra arms, extra hands, bad hands, duplicate character, multiple people"
)
...
def workflow(prefix,prompt,negative,seed,denoise):
    return {
      ...
      "17":{"class_type":"AnimaLLLiteApply","inputs":{"model":["10",0],"lllite_name":"anima-lllite-pose-1.safetensors","image":["15",0],"strength":1.75,"start_percent":0.0,"end_percent":1.0,"preserve_wrapper":True}},
      "7":{"class_type":"KSampler","inputs":{"seed":seed,"steps":24,"cfg":4.9,"sampler_name":"er_sde","scheduler":"simple","denoise":denoise,"model":["17",0],"positive":["5",0],"negative":["6",0],"latent_image":["4",0]}},
...
jobs=[]
for i,(seed,denoise) in enumerate([(27063101,0.54),(27063102,0.58),(27063103,0.62)],1):
    prefix=f'louise_frame02_one_third_raise_same_right_hand_cand{i:02d}'
    payload={'workflow':workflow(prefix,POS,NEG,seed,denoise),'prompt':POS,'negative_prompt':NEG,'seed':seed,'steps':24,'cfg':4.9,'width':W,'height':H,'sampler_name':'er_sde','scheduler':'simple','denoise':denoise,'lora':'posing-dynamics-anima.safetensors','lora_strength':0.65,'diffusion_model':'anima_preview3Base.safetensors','text_encoder':'qwen_3_06b_base.safetensors','vae':'qwen_image_vae.safetensors','image':SOURCE,'image_pose':POSE}
