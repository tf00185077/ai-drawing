...
# Keep watercolor preset identity, but remove silhouette/foreground-glow terms that produced black human-like blobs.
STYLE_PROMPT=(
...
"right hand holds a short straight wood-colored wand, plain wooden wand with no branches and no decoration, left hand empty, both arms lowered naturally, "
"large cartoon style European castle background only, huge complete European castle visible across the full frame, castle much larger than character, pointed spire towers, stone walls, rooftops, arched windows, full castle silhouette from left to right, full scene composition"
)
...
"left hand holding wand, wand switched hands, wrong hand holding wand, both hands holding wand, no wand, branch wand, forked wand, ornate wand, curved wand, thick wand, gem wand, glowing wand, staff, sword, second wand, "
"character centered, character too large, big character, close-up, upper body, cropped body, cropped feet, portrait, front view, back view, raised arm, spell casting, explosion, magic effect, night sky, "
"small castle, cropped castle, incomplete castle, castle only on one side, empty background, modern building, cluttered background, text, watermark, logo, signature, photorealistic, worst quality, low quality"
)
...
def workflow(prefix,prompt,negative,seed):
    return {
      ...
      "7":{"class_type":"KSampler","inputs":{"seed":seed,"steps":30,"cfg":5.3,"sampler_name":"er_sde","scheduler":"simple","denoise":1.0,"model":["15",0],"positive":["5",0],"negative":["6",0],"latent_image":["4",0]}},
...
jobs=[]
for i,seed in enumerate([27063501,27063502,27063503],1):
    prefix=f'louise_frame01_clean_no_extra_right_wand_cand{i:02d}'
    payload={'workflow':workflow(prefix,POS,NEG,seed),'prompt':POS,'negative_prompt':NEG,'seed':seed,'steps':30,'cfg':5.3,'width':W,'height':H,'sampler_name':'er_sde','scheduler':'simple','lora':'posing-dynamics-anima.safetensors','lora_strength':0.65,'diffusion_model':'anima_preview3Base.safetensors','text_encoder':'qwen_3_06b_base.safetensors','vae':'qwen_image_vae.safetensors','image_pose':POSE}
