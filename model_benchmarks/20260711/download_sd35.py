...
MANIFEST = Path('/Users/tf00185088/Desktop/ai-drawing/model_benchmarks/20260711/sd35_download_manifest.json')
STATE = MANIFEST.with_name('sd35_download_state.json')
...
def headers_for(r, range_start=None):
    ...
    elif 'huggingface.co' in r['url'] and hf_token:
...
def discover_sha(r):
    ...
    try:
        req = urllib.request.Request(r['url'], headers=headers_for(r), method='HEAD')
...
def download(r):
    ...
    with lock: save_state()
    req=urllib.request.Request(r['url'],headers=headers_for(r,start if start else None))
    try:
